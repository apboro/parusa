<template>
    <div class="dt-picker" @click.stop.prevent="false">
        <div class="dt-picker__controls">
            <span class="dt-picker__controls-previous" @click="clickNavigation(-1)"><icon-backward/></span>
            <span class="dt-picker__controls-display" v-if="mode==='date'">
                <span class="dt-picker__controls-display-item" @click="mode = 'month'">{{ monthToLocale(this.selected.month) }}</span>
                <span class="dt-picker__controls-display-item" @click="mode = 'year'">{{ selected.year }}</span>
            </span>
            <span class="dt-picker__controls-display" v-if="mode==='month'">
                <span class="dt-picker__controls-display-item" @click="mode = 'year'">{{ selected.year }}</span>
            </span>
            <span class="dt-picker__controls-display" v-if="mode==='year'">{{ selected.years + '-' + (selected.years + 19) }}</span>
            <span class="dt-picker__controls-next" @click="clickNavigation(1)"><icon-forward/></span>
        </div>

        <div class="dt-picker__calendar" :class="['dt-picker__calendar-'+mode]">
            <span v-for="(item, key) in getCalendar" :key="key"
                  class="dt-picker__calendar-item"
                  :class="[{
                    'dt-picker__calendar-item-disabled': !item['enabled'],
                    'dt-picker__calendar-item-current': !!item['current'],
                    'dt-picker__calendar-item-selected': !!item['selected'],
                    'dt-picker__calendar-item-preselected': !!item['preselected'],
                  }, item['class']]"
        ...
        <span class="dt-picker__current">Сегодня: <span class="dt-picker__current-link" @click="selectCurrent">{{ headerCurrent }}</span></span>
    </div>
</template>
