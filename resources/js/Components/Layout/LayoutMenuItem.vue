<template>
    <div class="application__menu-item"
         :class="['application__menu-item-'+level, active ? 'application__menu-item-active' : '']">
        <router-link v-if="route" class="application__menu-item-link" :to="{name:route}">{{ title }}</router-link>
        <span v-else class="application__menu-item-link">{{ title }}</span>
