<template>
    <LoadingProgress :loading="this.list.is_loading">
        <GuiContainer mt-15 mb-15 border p-20>
            ...
            <GuiContainer inline v-if="editable">
                    ...
                    <ListTableCell>
                        <template v-if="transaction['reason_raw']">
                            ...
                            <template v-if="transaction['reason_raw']['object'] === 'order'">
                                <router-link class="link" :to="{name: 'order-info', params: {id: transaction['reason_raw']['object_id'] }}">
        ...
        <GuiContainer mt-30>
            <GuiHeading text-lg>Статистика по счёту за период <span class="bold">{{ list.filters['date_from'] }} - {{ list.filters['date_to'] }}</span></GuiHeading>
