                ...
                <template v-for="order in list.list">
                    <ListTableRow :no-odd-even="true" :no-highlight="true">
                        <ListTableCell :class="'bold'">
                            <router-link class="link" :to="{name: 'return', params: {id: order['id']}}" v-html="highlight(order['id'])"/>
                        </ListTableCell>
                        <ListTableCell>
                            {{ order['date'] }}
                        </ListTableCell>
                        ...
                        <ListTableCell colspan="6">
                            <table class="details-table">
                                ...
                                <tr v-for="(ticket, key) in order['tickets']" :key="key">
                                    <td><span v-html="highlight(ticket['id'])"/></td>
                                    <td>
...
export default {
    ...
    methods: {
        ...
        makeReturn(order) {
            this.$router.push({name: 'return', params: {id: order['id']}, query: {return: 1}});
        },
