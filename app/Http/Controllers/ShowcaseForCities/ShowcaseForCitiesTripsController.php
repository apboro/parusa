...
namespace App\Http\Controllers\ShowcaseForCities;
...
class ShowcaseForCitiesTripsController extends ShowcaseTripsController
{
    public function trips(Request $request): JsonResponse
    {
        $partnerId = null;

        $date = $request->input('search.date') ?? now();
        ...
        $listQuery = Trip::saleTripQuery($partnerId === null)
            ...
            ->whereHas('excursion', function (Builder $query) use ($excursionId) {
                $query->where('id', $excursionId);
            })
        ...
        $trips = $trips->map(function (Trip $trip) use ($partnerId) {
            ...
            return [
                ...
                'seats' => $trip->additionalData?->with_seats ? $trip->getSeats() : null,
                ...
                'excursion_image_url' => $trip->excursion->images->isNotEmpty() ? $trip->excursion->images[0]?->url : null,
