...
class PromotersRegistryController extends ApiController
{
    ...
    public function list(ApiListRequest $request): JsonResponse
    {
        ...
        if ($request->input('search')){
            $query->whereHas('promoter', fn ($q) =>
            $q->where('name', 'like', '%'.$request->input('search').'%')
                ->orWhere('id', 'like', $request->input('search').'%'));
        }

        $workShifts = $query->paginate($request->perPage(10, $this->rememberKey));
        ...
        return APIResponse::list(
            $workShifts,
            ['ID', 'ФИО', 'Начало смены', 'Конец смены', 'За выход', 'Кол-во часов', 'Ставка', 'Почасовка', 'Оплата за время', 'Касса', '% от кассы', 'Всего начислено', 'Получено'],
            $filters,
            $this->defaultFilters)->withCookie(cookie($this->rememberKey, $request->getToRemember()));;
    }
