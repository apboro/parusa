...
class PromoterTripsSelectListController extends ApiController
{
                ...
                ->map(function (TicketRate $rate) use ($current) {
                    return [
                        ...
                        'value' => ($rate->partner_price
                            && ($current->isRepresentative() || $current->partnerId())
                            && $current->partner()->type_id !== PartnerType::promoter)
                            ? $rate->partner_price : $rate->base_price,
