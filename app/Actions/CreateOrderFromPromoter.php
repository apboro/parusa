...
class CreateOrderFromPromoter
{
    ...
    public function execute(array $tickets, array $customerData): Order
    {
        if (empty($tickets)) {
        ...
        foreach ($tickets as $ticket) {
            ...
            } else if ($ticket->base_price < $rate->min_price || $ticket->base_price > $rate->max_price && $ticket->trip->provider_id !== Provider::neva_travel) {
        ...
        // prepare order
        $order = new Order();
        $order->setStatus(OrderStatus::promoter_wait_for_pay, false);
        $order->setType(OrderType::promoter_sale, false);
