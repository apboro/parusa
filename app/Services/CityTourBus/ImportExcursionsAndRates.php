...
class ImportExcursionsAndRates
{
    public function run()
    {
        ...
        foreach ($cityTourExcursions as $externalExcursion) {
            ...
            $pier = Pier::firstOrCreate(['external_id' => $externalExcursion['id']]);
            $pier->name = $externalExcursion['address'];
            $pier->source = 'CityTour';
            $pier->save();
            $info = $pier->info;
            $info->address = $externalExcursion['address'];
            $info->save();

            $this->addOrUpdateGradesInDictionary($externalExcursion['tickets_categories']);

            $ratesList = $this->createOrUpdateRateList($foundExcursion ?? $newExcursion, $externalExcursion['tickets_categories']);
